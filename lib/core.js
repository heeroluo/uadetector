...
/** @typedef { import('../rules/types').Rule } Rule */
...
exports.execRules = function(ua, type, rules, verLength) {
  ...
  rules.some((r) => {
    ...
    if (r.regExp) {
      ...
      if (canMatch) {
        const version = handleVersion(RegExp.$1, r.verLength || verLength, r.version);
    ...
    if (canMatch) {
      result.name = r.name;
      if (r.extended) {
        if (typeof r.extended.isPC === 'function') {
          result.isPC = r.extended.isPC(ua);
        } else if ('isPC' in r.extended) {
          result.isPC = r.extended.isPC;
        }
      }
    }
